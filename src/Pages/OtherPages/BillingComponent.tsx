...
import {
  getBilling,
  getSubscribe,
} from "../../ReduxToolkit/Reducers/Change/Subscribe";
...
const BillingComponent = () => {
  ...
  const billingData = useSelector(
    (state: RootState) => state.subscribe.billing
  );
  ...
  const data: any = billingData?.data?.map((item: any, i: number) => {
  ...
  const calculateAnalytics = () => {
    ...
    const totalRevenue = data?.reduce(
      (sum: any, invoice: any) => sum + invoice.amount,
      0
    );
    const pendingPayments = data
      ?.filter((invoice: any) => invoice.status === "Unpaid")
      .reduce((sum: any, invoice: any) => sum + invoice.amount, 0);
        ...
        <Row className="mb-4">
          ...
          <Col md={3}>
            <AnalyticsCard
              ...
              value={`$${analytics.pendingPayments.toFixed(2)}`}
              icon={CreditCard}
              color="warning"
            />
          </Col>
